...
class IFCViewer {
  ...
  static CRS_DEFS = {
    ...
    // NAD83 / New York Long Island (ftUS) and its metric twin. Manhattan
    // sits essentially on the zone's central meridian, so convergence is
  ...
  readLengthUnit(ifc, modelID) {
    ...
    const resolve = (ref) => {
      ...
      const id = (ref && ref.value !== undefined) ? ref.value : ref;
      if (typeof id !== 'number') return (typeof ref === 'object') ? ref : null;
      try { return ifc.GetLine(modelID, id, true); } catch (e) { return null; }
    };

    const uaIds = ifc.GetLineIDsWithType(modelID, WebIFC.IFCUNITASSIGNMENT);
    if (!uaIds || !uaIds.size || uaIds.size() === 0) return;
    for (let a = 0; a < uaIds.size(); a++) {
      const ua = ifc.GetLine(modelID, uaIds.get(a), true);
      const units = ua && ua.Units ? ua.Units : [];
